	...
	if(isset($_POST['vendorDetailsVendorID'])) {
		...
		if(!empty($vendorDetailsVendorID)){
			...
			if(!empty($vendorDetailsVendorFullName) && !empty($vendorDetailsVendorMobile) && !empty($vendorDetailsVendorAddress)) {
				...
				if(empty($vendorDetailsVendorID)){
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Ingrese el ID del proveedor para actualizar ese proveedor. Puede encontrar el ID de proveedor usando la pestaña Buscar</div>';
					exit();
				}
				...
				if(isset($vendorDetailsVendorPhone2)){
					if(filter_var($vendorDetailsVendorPhone2, FILTER_VALIDATE_INT) === 0 || filter_var($vendorDetailsVendorPhone2, FILTER_VALIDATE_INT)) {
						...
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Ingrese un número válido para el número de teléfono 2.</div>';
						exit();
					}
				}
				...
				if($vendorIDSelectStatement->rowCount() > 0) {
					...
					$updateVendorDetailsSql = 'UPDATE vendor SET fullName = :fullName, email = :email, mobile = :mobile, phone2 = :phone2, address = :address, district = :district, status = :status WHERE vendorID = :vendorID';
					$updateVendorDetailsStatement = $conn->prepare($updateVendorDetailsSql);
					$updateVendorDetailsStatement->execute(['fullName' => $vendorDetailsVendorFullName, 'email' => $vendorDetailsVendorEmail, 'mobile' => $vendorDetailsVendorMobile, 'phone2' => $vendorDetailsVendorPhone2, 'address' => $vendorDetailsVendorAddress, 'district' => $vendorDetailsVendorDistrict, 'vendorID' => $vendorDetailsVendorID, 'status' => $vendorDetailsStatus]);
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Se actualizaron los detalles del proveedor.</div>';
					exit();
				} else {
					// vendorID is not in DB. Therefore, stop the update and quit
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>El ID del proveedor no existe en la base de datos. Por lo tanto, la actualización no es posible.</div>';
					exit();
				}
