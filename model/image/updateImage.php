				...
				if($itemNumberStatement->rowCount() > 0){
					...
					if(in_array($extension, $allowedTypes)){
						...
						if(move_uploaded_file($_FILES['itemImageFile']['tmp_name'], $targetPath)){
							...
							echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Imagen subida exitósamente</div>';
							exit();
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>El tipo de imagen no está permitido. Seleccione una imagen válida.</div>';
					exit();
					}
				}
