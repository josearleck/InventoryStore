	...
	if(isset($_POST['customerDetailsCustomerID'])) {
		...
		if(isset($customerDetailsCustomerFullName) && isset($customerDetailsCustomerMobile) && isset($customerDetailsCustomerAddress)) {
			...
			if(empty($customerDetailsCustomerID)){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Ingrese el ID Cliente para actualizar ese cliente.</div>';
				exit();
			}
			...
			if($customerIDSelectStatement->rowCount() > 0) {
				...
				$updateCustomerDetailsSql = 'UPDATE customer SET fullName = :fullName, email = :email, mobile = :mobile, phone2 = :phone2, address = :address, district = :district, status = :status WHERE customerID = :customerID';
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Datos del cliente actualizados.</div>';
				exit();
			} else {
				// CustomerID is not in DB. Therefore, stop the update and quit
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>ID Cliente no existe en DB. Por lo tanto, la actualización no es posible.</div>';
				exit();
			}
