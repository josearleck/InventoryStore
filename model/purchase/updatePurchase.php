	...
	if(isset($_POST['purchaseDetailsPurchaseID'])){
		...
		if(isset($purchaseDetailsItemNumber) && isset($purchaseDetailsPurchaseDate) && isset($purchaseDetailsQuantity) && isset($purchaseDetailsUnitPrice)){
			...
			if($purchaseDetailsPurchaseID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Ingrese un ID de compra</div>';
				exit();
			}
			...
			if($purchaseDetailsQuantity == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Ingrese la cantidad</div>';
				exit();
			}
			...
			if($originalPurchaseQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $purchaseDetailsItemNumber) {
					...
					if($newItemCurrentStockStatement->rowCount() < 1){
						...
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>El número de artículo no existe en la base de datos. Si desea actualizar este elemento, primero agréguelo a la base de datos.</div>';
						exit();
					}
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Se agregaron detalles de compra a la base de datos y se actualizaron los valores de las acciones.</div>';
					exit();
					...
					if($stockStatement->rowCount() > 0){
						...
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>El artículo no existe en la base de datos. Por lo tanto, primero ingrese este elemento en la base de datos usando el tab de <strong>Producto</strong></div>';
						exit();
					}	
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Los detalles de compra no existen en la base de datos para el ID de compra proporcionado. Por lo tanto, no se puede actualizar.</div>';
				exit();
