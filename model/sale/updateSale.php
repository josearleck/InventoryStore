	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
			...
			if(filter_var($saleDetailsQuantity, FILTER_VALIDATE_INT) === 0 || filter_var($saleDetailsQuantity, FILTER_VALIDATE_INT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Introduzca un número válido para Cantidad.</div>';
				exit();
			}
			...
			if(filter_var($saleDetailsUnitPrice, FILTER_VALIDATE_FLOAT) === 0.0 || filter_var($saleDetailsUnitPrice, FILTER_VALIDATE_FLOAT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Introduzca un número válido para precio unitario.</div>';
				exit();
			}
			...
			if($saleDetailsDiscount !== ''){
				...
				// Valid discount
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Introduce un número válido para Descuento.</div>';
					exit();
				}
			}
			...
			if($saleDetailsSaleID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Introduzca un ID de venta.</div>';
				exit();
			}
			...
			if($saleDetailsQuantity == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Por favor, introduzca la cantidad.</div>';
				exit();
			}
			...
			if($customerIDStatement->rowCount() < 1){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>El ID de cliente no existe en la base de datos. Introduzca un ID de cliente válido.</div>';
				exit();
			} else {
				$row = $customerIDStatement->fetch(PDO::FETCH_ASSOC);
				$customerID = $row['customerID'];
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					if($newItemCurrentStockStatement->rowCount() < 1){
						...
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>El número de artículo no existe en la base de datos. Si desea actualizar este elemento, agréguelo primero a la base de datos.</div>';
						exit();
					}
					...
					if($stockStatement->rowCount() > 0){
						...
						// sale details for it 
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>El elemento no existe en la base de datos. Por lo tanto, primero escriba este elemento a la base de datos mediante la ficha <strong>Artículo</strong>.</div>';
						exit();
					}	
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Los detalles de venta no existen en DB para el ID de venta dado. Por lo tanto, no se puede actualizar.</div>';
				exit();
